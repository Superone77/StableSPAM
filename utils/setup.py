...
from .modeling_llama import LlamaForCausalLM, LlamaMLPScaledSwiglu
...
from .base_linear import prepare_model_for_quest_training_simulation_act_weight
import bitsandbytes as bnb
...
def setup_model(args):
    ...
    if args.fp4 :
        ...
        logger.info('--'*20)
    if args.layernorm_scaling:
        print('Setting LayerNorm Scaling')
        for idx, block in enumerate(model.model.layers):
                orig_ln1 = block.input_layernorm
                block.input_layernorm = ScaledLayerNorm(orig_ln1, layer_idx = idx)
                orig_ln2 = block.post_attention_layernorm
                block.post_attention_layernorm = ScaledLayerNorm(orig_ln2, layer_idx = idx)
    if args.smoothswiglu:
        print('Setting SmoothSwiGLU')
        for idx, block in enumerate(model.model.layers):
            ori_mlp = block.mlp
            block.mlp = LlamaMLPScaledSwiglu(ori_mlp)
...
def setup_optimization(args, model, trainable_params, param_groups, id_galore_params,model_config=None):
    ...
    if args.optimizer.lower() == "adam":
        ...
        optimizer = Adam8bitSQNR(trainable_params, lr=args.lr, weight_decay=args.weight_decay)
    elif args.optimizer.lower() == "adamfp8":
